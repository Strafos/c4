...
class connect4:
    ...
    def __init__(self):
        self.gamestate = [[' ' for i in range(self.width)] for i in range(self.height)]
    ...
    def win(self, coord, piece):
        ...
        for dy in dir:
            for dx in dir:
                ...
                ystart = coord[1]
                for i in range(3):
                    if xstart > 0 and xstart <= self.width - 2 and ystart > 0 and ystart <= self.height - 2:
                        xstart -= dx
                        ystart -= dy
                count = 0
                for i in range(self.connect*2 -1):
                    posx = xstart + i*dx
                    posy = ystart + i*dy
                    if posx > 0 and posx <= self.width - 2 and posy > 0 and posy <= self.height - 2:
